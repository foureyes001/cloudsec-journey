...
2026-08-27 | #78 | Subsets | COLD PASS — one mechanical defect, assistant-flagged | fixed 7/7 + shape 2/2 in VS Code after fix; SAN: PENDING (WSL not installed) | Include/exclude recursion, n passed BY VALUE so no pop needed; empty subset seeded in v; each non-empty subset emitted once at its highest-index element. Structure correct on first write. Defect: base case written as i == v.size() (output accumulator) instead of i == nums.size() — out-of-range read on nums[i]; would have failed case 1 had the harness been run before submitting. Residuals: i is int vs size_t = 6th signed/unsigned appearance; nums passed by value = full copy at each of 2^n nodes, should be const&. Time O(n*2^n), space O(n) recursion depth. RETIRED from Tier 1 -> Tier 1 now 0.